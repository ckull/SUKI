        ...
        if(args[0] === 'album'){
            ...
            .then( data => {
              data.body.albums.items.forEach(item => {
                embed["fields"].push({name: item.artists[0].name, value: `[${item.name}](${item.external_urls.spotify})`})
              });
            ...
            var input = args.splice(1).join(" ")
            spotifyApi.searchArtists(input, {limit: 5})
            .then( data => {
...
exports.help = {
    ...
    value: "Use spotify function inside discord! \n\nAvailable command:\n" + "```fix\nalbum\ntrack\nartist```"
}
